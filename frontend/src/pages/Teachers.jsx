  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this teacher?')) {
      try {
        ...
        alert('Failed to delete teacher. It may be referenced by other records.');
      }
    }
  };
  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-700 to-purple-600">Teachers</h1>
        <button
          onClick={() => openModal()}
        ...
        <table className="min-w-full divide-y divide-indigo-50">
          ...
          <tbody className="bg-transparent divide-y divide-indigo-50">
            {teachers.map((teacher) => (
              <tr key={teacher.id} className="hover:bg-indigo-50/30 transition-colors">
                <td className="px-6 py-4 whitespace-nowrap text-sm text-indigo-900 font-medium">{teacher.name}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-indigo-700">{teacher.email}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-indigo-700">{teacher.contact}</td>
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <button onClick={() => openModal(teacher)} className="text-indigo-600 hover:text-indigo-900 mr-3 bg-indigo-50 p-2 rounded-lg hover:bg-indigo-100 transition-colors">
                    <Edit2 className="w-4 h-4" />
                  </button>
                  <button onClick={() => handleDelete(teacher.id)} className="text-red-500 hover:text-red-700 bg-red-50 p-2 rounded-lg hover:bg-red-100 transition-colors">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-indigo-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl max-w-md w-full p-8 border border-white/50">
            <h2 className="text-2xl font-bold text-indigo-900 mb-6">{editingTeacher ? 'Edit Teacher' : 'Add Teacher'}</h2>
            <form onSubmit={handleSubmit}>
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-semibold text-indigo-900/80">Email</label>
                  <input
                    type="email"
                    required
